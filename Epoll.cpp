...
{
    m_events.resize(fdNumber);
    while (true)
    {
        nReady = epoll_wait(m_epollfd, &*m_events.begin(), fdNumber, timeout);
        if (nReady == 0)
...
void* Epoll::getEventClient(int eventIndex){
    if (eventIndex > nReady)
        throw EpollException("parameter(s) error");
    return m_events[eventIndex].data.ptr;
}
